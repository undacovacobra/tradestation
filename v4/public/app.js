...
async function refresh() {
  try {
    ...
    document.querySelector("#pools").innerHTML = data.pools.map((p) => {
      ...
      return `<article><div class="row"><h3>${esc(p.name)}</h3><span class="tag">${esc(p.id)}</span></div><p>${p.accounts.length} active account${p.accounts.length === 1 ? "" : "s"}</p><dl><dt>State</dt><dd>${open ? "Position recorded" : "Flat"}</dd><dt>Account</dt><dd>${esc(open?.accountName || "—")}</dd><dt>Next</dt><dd>${esc(p.state?.nextAccountId || p.accounts[0]?.id || "—")}</dd></dl></article>`;
