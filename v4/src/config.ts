...
export const config = {
  ...
  registryPath: resolve(ROOT, process.env.REGISTRY_PATH ?? "data/registry.json"),
  poolStateDir: resolve(ROOT, process.env.POOL_STATE_DIR ?? "data/pools"),
  healthCheckSeconds: Math.max(10, Number(process.env.HEALTH_CHECK_SECONDS ?? 45)),
  /** Default only. Each connection may override this in registry.json. */
  accountIdPattern: new RegExp(process.env.ACCOUNT_ID_PATTERN ?? "[A-Z0-9][A-Z0-9_-]{5,}"),
};
